...
model_distribution = normrnd(fits(1), fits(2), [1,length(RTs)]);

%histogram(RTs)
%hold on
%histogram(model_distribution)

real_cdf = cdf('Normal', 1./RTs, muR, deltaS);
model_cdf = cdf('Normal', model_distribution, fits(1), fits(2));

plot(RTs, real_cdf)
hold on
plot(model_distribution, model_cdf)

% Compute the CDFs for both datasets
%[cdf1, x1] = ecdf(real_cdf);
%[cdf2, x2] = ecdf(model_cdf);

% Plot CDF for the first dataset
%[cdf1, x1] = cdfplot(real_cdf);
%plot(x1, cdf1, 'b', 'LineWidth', 2);
%hold on
% Plot CDF for the second dataset
%[cdf2, x2] = cdfplot(model_cdf);
%plot(x2, cdf2, 'r', 'LineWidth', ...


% Sort the data for plotting the CDF
real_cdf = sort(real_cdf);
model_cdf = sort(model_cdf);

% Compute the CDF values for both datasets
cdf1 = (1:length(real_cdf)) / length(real_cdf);
cdf2 = (1:length(model_cdf)) / length(model_cdf);
...
% Plot CDF for the first dataset
plot(real_cdf, cdf1, 'b', 'LineWidth', 2);

% Plot CDF for the second dataset
plot(model_cdf, cdf2, 'r', 'LineWidth', 2);
